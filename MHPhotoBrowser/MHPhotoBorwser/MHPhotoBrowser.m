...
#import "MHPhotoBrowserView.h"
@implementation MHPhotoBrowser
{
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.backgroundColor = MHPhotoBrowserBackgrounColor;
    }
    return self;
}
//当视图移动完成后调用
...
- (void)setupScrollView
{
    ...
    for (int i = 0; i < self.imageCount; i++) {
        MHPhotoBrowserView *view = [[MHPhotoBrowserView alloc] init];
...
- (void)saveImage
{
    ...
    MHPhotoBrowserView *currentView = _scrollView.subviews[index];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    if (error) {
        label.text = MHPhotoBrowserSaveImageFailText;
    }   else {
        label.text = MHPhotoBrowserSaveImageSuccessText;
    }
...
{
    MHPhotoBrowserView *view = _scrollView.subviews[index];
...
- (void)layoutSubviews
{
    ...
    rect.size.width += MHPhotoBrowserImageViewMargin * 2;
    _scrollView.bounds = rect;
    //    _scrollView.center = self.center;
    _scrollView.center = CGPointMake(self.bounds.size.width *0.5, self.bounds.size.height *0.5);
    
    CGFloat y = 0;
    __block CGFloat w = _scrollView.frame.size.width - MHPhotoBrowserImageViewMargin * 2;
    CGFloat h = _scrollView.frame.size.height;
    
    [_scrollView.subviews enumerateObjectsUsingBlock:^(MHPhotoBrowserView *obj, NSUInteger idx, BOOL *stop) {
        CGFloat x = MHPhotoBrowserImageViewMargin + idx * (MHPhotoBrowserImageViewMargin * 2 + w);
...
- (void)show
{
    ...
    _contentView.backgroundColor = MHPhotoBrowserBackgrounColor;
    ...
    [self performSelector:@selector(onDeviceOrientationChangeWithObserver) withObject:nil afterDelay:MHPhotoBrowserShowImageAnimationDuration + 0.2];
...
//    NSLog(@"%@",NSStringFromCGRect(rect));
    ...
    [UIView animateWithDuration:MHPhotoBrowserShowImageAnimationDuration animations:^{
...
{
    MHPhotoBrowserView *currentView = _scrollView.subviews[self.currentImageIndex];
...
{
    MHPhotoBrowserView *view = (MHPhotoBrowserView *)recognizer.view;
    ...
    [UIView animateWithDuration:MHPhotoBrowserHideImageAnimationDuration animations:^{
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    for (MHPhotoBrowserView *view in _scrollView.subviews) {
