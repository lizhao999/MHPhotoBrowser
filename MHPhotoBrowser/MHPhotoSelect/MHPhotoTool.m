...
#import "MHPhotoSelectConfig.h"
 @implementation MHPhotoTool

static MHPhotoTool *sharePhotoTool = nil;
...
{
    NSMutableArray<MHPhotoAblumList *> *photoAblumList = [NSMutableArray array];
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if(collection.assetCollectionSubtype != 202 && collection.assetCollectionSubtype < 212){
            ...
            if (assets.count > 0) {
                MHPhotoAblumList *ablum = [[MHPhotoAblumList alloc] init];
...
//    CGFloat width = MIN(kViewWidth, kMaxImageWidth);
    if (requestID >= 1 && size.width/kViewWidth==scale) {
...
- (void)requestImageForAsset:(PHAsset *)asset size:(CGSize)size resizeMode:(PHImageRequestOptionsResizeMode)resizeMode progress:(void (^)(double))progressHandler completion:(void (^)(UIImage *image, NSDictionary *info))completion
{
    //请求大图界面，当切换图片时，取消上一张图片的请求，对于iCloud端的图片，可以节省流量
    static PHImageRequestID requestID = -1;
    CGFloat scale = [UIScreen mainScreen].scale;
...
#pragma mark - 判断图片是否存储在本地/或者已经从iCloud上下载到本地
